        ...
        if (matterHandler == null) {
            ...
            for (int i = 0; i < matterHandler.size(); i++) {
                ...
                if (accessor.getPlayer().isShiftKeyDown()) {
                    ...
                    tag.putString("matterStorageAmount_" + i, matterHandler.getStored(i).getAmount().toComponent(true, true).getString() + (type == null ? "mB" : type.getSuffix()));
                    tag.putString("matterStorageCapacity_" + i, matterHandler.getMaxStored(i).toComponent(true, true).getString() + (type == null ? "mB" : type.getSuffix()));
                }
            }
        }
    }
